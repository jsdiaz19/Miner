...
class Game:
	...
	def drawScene(self):
		...
		self.enemy3.wallFollower(self.player,posX,posY)
		self.screen.blit(self.enemy3.image,((self.enemy3.posX-self.CameraX)*SCALE,(self.enemy3.posY-self.CameraY)*SCALE))
	...
	def run(self):
		...
		while self.playing:
			...
			if(matriz[posX][posY] == 'K'):
				skrn = pygame.display.set_mode((600,440))
				...
				while True:
					for event in pygame.event.get():
						...
						rendered = sys_font.render('GANASTE!',0,(80,200,80))
				...
				while True:
					for event in pygame.event.get():
						if event.type == pygame.QUIT:
							...
							sys.exit()
						sys_font = pygame.font.SysFont("None", 60)
						rendered = sys_font.render('PERDISTE',0,(80,200,80))
						skrn.blit(rendered, (180,100))
						pygame.display.update()
...
class  enemy3:
	...
	def collideWithWalls(self, dx, dy):
		ans = 0
		if matriz[int(self.posX-dx)][int(self.posY)]=='#':# horizontales
			ans = 1	
		if matriz[int(self.posX)][int(self.posY-dy)]=='#':# verticales
			ans = 2	
		return ans

	def wallFollower(self, player,PlayerX,PlayerY):
		dist= self.distance(PlayerY,PlayerX,self.posX,self.posY)
		if(dist<=1):
			if self.neighbors(PlayerY,PlayerX,self.posX,self.posY):
				self.image= pygame.image.load("sprites/attack.png") 
		...
		if(dist>= 1 and dist < 2):
			...
			dx, dy = self.posX - posY, self.posY - posX
			dist = math.hypot(dx, dy)
			dx, dy = dx / dist, dy / dist
			#print(dx,dy)
			if(dx >= 0 and dy >= 0):#Izq
				self.image= pygame.transform.rotate(self.image,-self.angle)
				self.image= pygame.transform.rotate(self.image,0)
				self.angle=0
			if(dx <= 0 and dy >= 0):#Der
				self.image= pygame.transform.rotate(self.image,-self.angle)
				self.image= pygame.transform.rotate(self.image,180)
				self.angle=180
			#if(dx < 0 and dy == 0 and ):#Arr
			#	self.image= pygame.transform.rotate(self.image,-self.angle)
			#	self.image= pygame.transform.rotate(self.image,270)
			#	self.angle=270
			if(dx <= 0 and dy <= 0):#Aba
				self.image= pygame.transform.rotate(self.image,-self.angle)
				self.image= pygame.transform.rotate(self.image,90)
				self.angle=90
			self.posX -= dx*0.8
